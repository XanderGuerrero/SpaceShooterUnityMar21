...
public class PlayerManagerTest : MonoBehaviour
{
    ...
    private void checkShooting()
    {
        ...
        if ((Input.GetButtonDown("Fire4")) && (!shootingCoolDownMissile.IsCoolDownOn()) && target.enemyName != string.Empty)
        {
            ...
            if (alternateBulletShotSpawn == 0)
            {
                ...
                GameObject obj = target.visibleTargets.Find(item => item.name == target.enemyName).gameObject;
    ...
    private void checkMovement()
    {
        ...
        {
            engLight.intensity = Mathf.Lerp(engLight.intensity, 1f, Time.deltaTime * NetworkClient.SERVER_UPDATE_TIME);
        }
    }
